...
class Physics:
    ...
    def calc_ball_velocity(ball: Ball, paddle: Paddle) -> Velocity:
        ...
        if Physics.__ball_paddle_intersection(ball, paddle)[0]:
            bounce_angle = Physics.__ball_paddle_intersection(ball, paddle)[1] * 90
    ...
    @staticmethod
    def __ball_paddle_intersection(ball: Ball, paddle: Paddle) -> [bool, float]:
        """
            Calculates the intersection of the ball with the paddle.
            Returns:
        ...
        if ball.x_pos + ball.m_radius == paddle.x_pos and ball.y_pos == paddle.y_pos:  # right paddle intersection
            collision = True
        if ball.x_pos - ball.m_radius == paddle.x_pos + paddle.m_size[0] and ball.y_pos == paddle.y_pos:  # left paddle intersection
            collision = True
        if collision:
